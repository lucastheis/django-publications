...
if StrictVersion(django.get_version()) >= StrictVersion('1.7.0'):
	...
	setup()
	sys.exit(DiscoverRunner(verbosity=1).run_tests(['publications-bootstrap']))
else:
	from django.test.simple import DjangoTestSuiteRunner
	sys.exit(DjangoTestSuiteRunner(verbosity=1).run_tests(['publications-bootstrap.Tests', 'publications-bootstrap.LiveTests']))
